...
import termsData from '@/data/termsContent.json';
          ...
          <div className="bg-card rounded-lg shadow-sm border p-8 md:p-12">
            ...
            <div className="mb-12">
              ...
              <div className="grid md:grid-cols-2 gap-2 text-sm">
                <div className="space-y-2">
                  {termsData.sections.slice(0, Math.ceil(termsData.sections.length / 2)).map((section) => (
                    <a 
                      key={section.id}
                      href={`#${section.id}`} 
                ...
                <div className="space-y-2">
                  {termsData.sections.slice(Math.ceil(termsData.sections.length / 2)).map((section) => (
                    <a 
                      key={section.id}
                      href={`#${section.id}`} 
                      className="block text-primary hover:text-primary/80 transition-colors"
                ...
                return (
                  <section key={section.id} id={section.id} className="mb-12">
                    ...
                    <div className="space-y-4 text-muted-foreground">
                      {(() => {
                        // Handle plain-language section with bullet points
                        if (section.id === 'plain-language') {
                          return (
            ...
            <div className="mt-12 pt-8 border-t border-border">
              <p className="text-sm text-muted-foreground text-center">
                Last updated: [Insert date] | For questions about these terms, contact support@makeachangefoundation.in
              </p>
            </div>
